	...
	***************************************************************************/
	...
	function checkuser($user_email, $password, $pageDir){
        ...
        $password = $this->generatePassword(6);
				...
				mail($user_email, WELCOME_TITLE, $message, "From: ".FROM."\r\n");

				$ok = "Un email a été envoyé à l'adresse $user_email.";
				return $ok;
			}else{
				$err = LOGIN_UNKNOWN;
				return $err;
		...
		// login OK
		  ...
		  if($pageDir=="") $pageDir = "index.php";
			header("Location: http://" . $_SERVER['HTTP_HOST'] . "/$pageDir");
			exit();
		}
	}
	...
	***************************************************************************/
	function getPasswd($user_email){
      ...
      $msg_mdp = sprintf(PASSWD_MSG,$user_first_name,$user_name,$user_email,$password);
			mail($user_email, PASSWD_TITLE, $msg_mdp ,"From: ".FROM."\r\n");
	...
	***************************************************************************/
	function userInfos($user_id){
		...
		$req = "
			...
			FROM user_bill b, user_ship s, user u
			WHERE s.user_id=u.user_id and b.user_id=u.user_id
