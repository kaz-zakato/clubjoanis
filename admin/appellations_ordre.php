...
<span class="titre"><br>
<b>Administration de l'ordre des appellations</b>
<br>
<br>
</span>
<?php
...
$numVin=explode(",",$ChaineTitre);
	For ($i=0;$i<Count($numVin);$i++)
	{
		$CleN=$numVin[$i];
		$ordre = $i + 1;
		mysql_query("update LOW_PRIORITY product_region set product_region_ordre='$ordre' where product_region_id='$CleN'",$dbh);
	}
...
else
{
	...
	$id=mysql_query("SELECT * FROM product_region WHERE product_region_nom != 'Aucun' ORDER by product_region_ordre",$dbh);
...
<?
	For ($j=0;$j<$nump;$j++)
	{
	$N=mysql_result($id,$j,"product_region_id");
	//$vins=Stripslashes(substr(mysql_result($id,$j,"product_region_nom"),0,150));
	$appellation=Stripslashes(substr(mysql_result($id,$j,"product_region_nom"),0,150));
...
<br>
<form Name="FormOrdre" Action="appellations_ordre.php?list=1&ValidationOrdre=Oui" Method="Post">
