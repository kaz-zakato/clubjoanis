...
if(isset($_POST["action"])){
	...
	*****************************************************************************/
		...
		$sql_photo = $db->query("SELECT product_full_img FROM product WHERE product_id=".$_POST["product_id"]);
		$photo = $dir;
		$photo .= mysqli_result($sql_photo,0,"product_full_img");
		$photo2 = $dir;
		$photo2 .= "mini/";
		$photo2 .= mysqli_result($sql_photo,0,"product_full_img");
		if(!$db->query($SQL)) print "erreur SQL"; 
		else 
		{
		unlink($photo);
		unlink($photo2);
		print "<font color=\"#7c3c3e\"><strong>Suppression du produit effectuée !</strong></font>";
		}
	}
	/*****************************************************************************
	* file upload (image)
	*****************************************************************************/
	$product_full_img = $_FILES["product_full_img"];
  echo "full img : ".$_FILES["product_full_img"]."<br/>";
  echo "full img name : ".$_FILES["product_full_img"]['name']."<br/>";
  echo "full img type : ".$_FILES["product_full_img"]['type']."<br/>";
  echo "img : ".$img;
  if ($product_full_img!=null && $product_full_img['name']!=""){
		if ($product_full_img['type'] == "image/gif" || $product_full_img['type'] == "image/pjpeg" || $product_full_img['type'] == "image/jpeg" || $product_full_img['type'] == "image/png"){
...
<table width="100%" style="color:white">
  ...
  <tr>
    <td>
      <span style="color: #7c3c3e;font-weight:bold;">Statut</span>
    </td>
    <td></td>
    <td>
  ...
  <tr>
    <td>
      <span style="color: #7c3c3e; font-weight:bold;">En page d'accueil</span>
    </td>
    <td></td>
    <td>
  ...
  <tr>
    <td>
      <span style="color: #7c3c3e; font-weight:bold;">Idée cadeau</span>
    </td>
    <td></td>
    <td>
  ...
  <td>
  <p><br>
  	  <input type="hidden" name="product_ordre" value ="<?php if(isset($product_ordre)) echo $product_ordre; else echo "25"; ?>" />		
